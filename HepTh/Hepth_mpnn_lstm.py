...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 11 19:05:17 2023
...
total_vertices=[0 for i in range(total_graphs)]
common_vertices_list=[0 for i in range(num_groups)]
...
for group_num in range(num_groups+1):
    ...
    for i in range(start_idx, end_idx):
        with open(f'ca-cit-HepTh_sorted_adj_list_{i}.txt', 'r') as f:
            lines = f.readlines()
            for line in lines:
                node_id, *neighbors = map(int, line.strip().split())
                total_vertices[i]+=len(neighbors)+1
...
##for counting total number of common nodes
...
def compute_common_exclusive (group_adj_lists, i):
    ...
    num_common_vertices=len(common_node_ids)
    ...
    return common_adj_list, exclusive_adj_list_1, exclusive_adj_list_2, timestamp_considered, num_common_vertices
...
def process_BacthNorm(ready_RNN_cnt, timestamp):
    input_layer_dim=RNN_input_dim ## 16to match with GNNAdvsior   64 used originally## hidden layer dim
    PE_array_dim=16 ## 16x16 PE array
    MACs_per_CPE=4
    num_vertices=ready_RNN_cnt
    #input_buffer_size= buffer_capacity
    sub_vector_len=math.ceil(input_layer_dim/PE_array_dim)
    cycle_required_per_node=math.ceil(sub_vector_len/MACs_per_CPE) #58 ## by celing we are adding one more cycle that may be required by ReLU
    return cycle_required_per_node*num_vertices
...
# Iterate over timestamps
    ...
    for timestamp in timestamp_range:
        ...
        ## for calculating the batch normalization cycle
        BatchNorm_cycle_dict[timestamp]=process_BacthNorm(ready_RNN_dict[timestamp], timestamp)
    return edge_processed_dict, subgraph, cache_miss_cnt_dict, RNN_cycle_dict, BatchNorm_cycle_dict
...
BatchNorm_cycle_list_per_group_top=[] 

## for computing the indices i for the creation of common, exclusive_1 and exclusive_2
for i in range(0,len(group_adj_lists)-1,2):
## for each i compute commong, ex_1, ex_2
    common_adj_list, exclusive_adj_list_1, exclusive_adj_list_2, timestamp_considered, num_common_vertices = compute_common_exclusive(group_adj_lists,i)  
    #get the current time stamps under consideration
    timestamp_range=timestamp_considered
    common_vertices_list[i]+=num_common_vertices
    ...
    total_common_nodes+=len(common_adj_list.keys())
    ...
    if (total_DRAM_access_required > max_features_in_buffer):
          ...
          for iter in range(10):
              ...
              edge_processed_dict, subgraph, cache_miss_cnt_dict, RNN_cycle_dict, BatchNorm_cycle_dict = process_current_subgraph(subgraph, timestamp_range)
              ...
              BatchNorm_cycle_per_iteration=0
              for timestamp, BatchNorm_cycle_cnt in BatchNorm_cycle_dict.items():
                  BatchNorm_cycle_per_iteration += BatchNorm_cycle_cnt
              BatchNorm_cycle_per_group.append(BatchNorm_cycle_per_iteration)
          ...
          ## for updating the BatchNorm cycle count list at top
          BatchNorm_cycle_list_per_group_top.append(sum(BatchNorm_cycle_per_group))
        ...
        ## for computing the batch noramalization cycles
        BatchNorm_cycle_per_iteration=0
        for timestamp, BatchNorm_cycle_cnt in BatchNorm_cycle_dict.items():
            BatchNorm_cycle_per_iteration += BatchNorm_cycle_cnt
        BatchNorm_cycle_list_per_group_top.append(BatchNorm_cycle_per_iteration)
        
        
total_processing_cycle=2*sum(GNN_cycle_list_per_group_top)+sum(off_chip_cycle_list_per_group_top)+2*sum(RNN_cycle_list_per_group_top)+2*sum(BatchNorm_cycle_list_per_group_top)
...
ratio_pre_process_inference=sum(time_list)/total_processing_time